...
class ChatView(View):
    def get(self, request):
        return render(request, "nautobot_chatbot/chat.html", {"chatbot_title": "Ben Bot Assistant"})
...
def chat_api(request):
    try:
        ...
        session, created = ChatSession.objects.get_or_create(
            session_id=session_id, defaults={"user": request.user, "is_active": True}
        )
...
def chat_history(request):
    ...
    if session_id:
        messages = ChatMessage.objects.filter(user=request.user, session_id=session_id).order_by(
            "timestamp"
        )[:50]
    else:
        ...
        if latest_session:
            messages = ChatMessage.objects.filter(
                user=request.user, session_id=latest_session.session_id
    ...
    try:
        ...
        if session:
            ...
            session = ChatSession.objects.create(
                user=request.user, session_id=session_id, is_active=True
            )
            return JsonResponse(
                {
                    "session_id": session.session_id,
...
def ai_status(request):
    ...
    try:
        ...
        return JsonResponse({"error": str(e), "ai_enabled": False, "status": "error"}, status=500)
