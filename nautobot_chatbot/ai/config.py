...
class AIConfig:
    ...
    def get_ai_config() -> Dict[str, Any]:
        ...
        config = {
            "enabled": django_ai_config.get(
                "enabled", os.getenv("AI_PROVIDER_ENABLED", "false").lower() == "true"
            ),
            "provider": django_ai_config.get("provider", os.getenv("AI_PROVIDER", "generic")),
            "api_base_url": django_ai_config.get("api_base_url", os.getenv("AI_API_ENDPOINT", "")),
            "api_key": django_ai_config.get("api_key", os.getenv("AI_API_KEY", "")),
            "model_name": django_ai_config.get("model_name", os.getenv("AI_MODEL_NAME", "default")),
            "temperature": django_ai_config.get(
                "temperature", float(os.getenv("AI_TEMPERATURE", "0.7"))
            ),
            "max_tokens": django_ai_config.get(
                "max_tokens", int(os.getenv("AI_MAX_TOKENS", "2000"))
            ),
            "timeout": django_ai_config.get("timeout", int(os.getenv("AI_REQUEST_TIMEOUT", "30"))),
    ...
    def get_rag_config() -> Dict[str, Any]:
        ...
        return {
            "enabled": django_rag_config.get(
                "enabled", os.getenv("RAG_ENABLED", "true").lower() == "true"
            ),
            "embedding_provider": django_rag_config.get(
                "embedding_provider", os.getenv("RAG_EMBEDDING_PROVIDER", "sentence_transformers")
            ),
            "embedding_model": django_rag_config.get(
                "embedding_model", os.getenv("RAG_EMBEDDING_MODEL", "all-MiniLM-L6-v2")
            ),
            "chunk_size": django_rag_config.get(
                "chunk_size", int(os.getenv("RAG_CHUNK_SIZE", "1000"))
            ),
            "chunk_overlap": django_rag_config.get(
                "chunk_overlap", int(os.getenv("RAG_CHUNK_OVERLAP", "200"))
            ),
            "max_results": django_rag_config.get(
                "max_results", int(os.getenv("RAG_MAX_CHUNKS", "5"))
            ),
            "similarity_threshold": django_rag_config.get(
                "similarity_threshold", float(os.getenv("RAG_SIMILARITY_THRESHOLD", "0.7"))
            ),
            "documents_path": django_rag_config.get(
                "documents_path", os.getenv("RAG_DOCUMENTS_PATH", default_docs_path)
            ),
            "vector_store_path": django_rag_config.get(
                "vector_store_path", os.getenv("VECTOR_STORE_PATH", default_vector_path)
            ),
            # Documentation sources
            "doc_sources": django_rag_config.get(
                "doc_sources",
                {
    ...
    def get_mcp_config() -> Dict[str, Any]:
        ...
        return {
            "enabled": django_mcp_config.get(
                "enabled", os.getenv("MCP_TOOLS_ENABLED", "true").lower() == "true"
            ),
            "enable_navigation": django_mcp_config.get(
                "enable_navigation", os.getenv("MCP_NAVIGATION_ENABLED", "true").lower() == "true"
            ),
            "enable_api_queries": django_mcp_config.get(
                "enable_api_queries", os.getenv("MCP_API_QUERIES_ENABLED", "true").lower() == "true"
            ),
            "api_rate_limit": django_mcp_config.get(
                "api_rate_limit", int(os.getenv("MCP_API_RATE_LIMIT", "10"))
            ),
            "enabled_tools": django_mcp_config.get(
                "enabled_tools", ["navigate_to_page", "search_nautobot", "create_object"]
            ),
            # Nautobot API configuration
            "nautobot_api_base": django_mcp_config.get(
                "nautobot_api_base", os.getenv("NAUTOBOT_API_BASE", "http://localhost:8000/api")
            ),
            "nautobot_api_token": django_mcp_config.get(
                "nautobot_api_token", os.getenv("NAUTOBOT_API_TOKEN", "")
            ),
        }
        ...
        try:
            ...
            if mcp_config["enabled"]:
                if not mcp_config["nautobot_api_base"]:
                    results["warnings"].append(
                        "MCP enabled but no Nautobot API base URL configured"
                    )
            ...
            results["components_status"] = {
                ...
                and not any("AI" in error for error in results["errors"]),
                "rag_enabled": rag_config["enabled"]
                and not any("RAG" in error for error in results["errors"]),
                "mcp_enabled": mcp_config["enabled"]
                and not any("MCP" in error for error in results["errors"]),
