...
class GenericLLMClient:
    ...
    def is_available(self) -> bool:
        ...
        return (
            ...
            and self.config.get("api_endpoint")
            and self.config.get("model_name")
        )
    ...
    def generate_response(
        ...
        try:
            ...
            response = self.session.post(
                self.config["api_endpoint"], json=payload, timeout=self.config.get("timeout", 30)
            )
    ...
    def _build_system_prompt(
        self, context: Optional[str] = None, tools: Optional[List[Dict]] = None
    ) -> str:
        """
        ...
        if context:
            system_parts.extend(["", "Relevant documentation context:", context.strip()])

        # Add available tools if any
        if tools:
            system_parts.extend(["", "Available tools:", json.dumps(tools, indent=2)])
