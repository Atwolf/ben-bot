...
class AIEngine:
    ...
    def generate_response(
        self, message: str, user: Any, session: Optional[Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.llm_client and self.llm_client.is_available():
                ...
                response_data.update(
                    self._generate_ai_response(message, context, tool_results, user)
                )
            else:
                # Fallback to rule-based response with enhanced capabilities
                response_data.update(
                    self._generate_fallback_response(message, context, tool_results, user)
                )
        ...
        for phrase, page_type in page_mappings.items():
            if phrase in message_lower:
                ...
                if result.get("success"):
                    return {"tool": "navigate_to_page", "result": result}
    ...
    def _try_create_tool(self, message_lower: str) -> Optional[Dict[str, Any]]:
        ...
        for phrase, object_type in object_mappings.items():
            if phrase in message_lower:
                ...
                if result.get("success"):
                    return {"tool": "create_object", "result": result}
    ...
    def _try_search_tool(self, message: str, message_lower: str) -> Optional[Dict[str, Any]]:
        ...
        if query and query != message.lower():
            ...
            if result.get("success"):
                return {"tool": "search_nautobot", "result": result}

        return None

    def _generate_ai_response(
        self, message: str, context: str, tool_results: List[Dict[str, Any]], user: Any
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            llm_response = self.llm_client.generate_response(
                messages=messages, context=context, tools=tools_schema
            )
            ...
            return {
                "text": llm_response.get(
                    "text", "I apologize, but I could not generate a response."
                ),
                "actions": actions,
    ...
    def _generate_fallback_response(
        ...
        if actions:
            if any("navigate" in action.get("type", "") for action in actions):
                response_text = (
                    "I can help you navigate to the relevant page. Click the button below:"
                )
            else:
            ...
            if "device" in message_lower:
                ...
                response_text = (
                    "Here's relevant information from the documentation: " + context[:200] + "..."
                )
        else:
        ...
        return {
            ...
            "components": {
                ...
                "mcp_tools": {
                    ...
                    "available_tools": (
                        list(self.mcp_registry.tools.keys()) if self.mcp_registry else []
                    ),
                },
                "api_client": {
